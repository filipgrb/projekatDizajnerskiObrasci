...
public class ModifyLineDialog extends JDialog {
	...
	private JTextField txtXStart, txtYStart, txtXEnd, txtYEnd;
		...
		btnOk.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent arg0) {
				if (txtXStart.getText().trim().length() == 0) {
					JOptionPane.showMessageDialog(getContentPane(), "Enter X Start", "Error!", JOptionPane.WARNING_MESSAGE);
					return;
				} else if (txtYStart.getText().trim().length() == 0) {
					JOptionPane.showMessageDialog(getContentPane(), "Enter Y Start!", "Error!", JOptionPane.WARNING_MESSAGE);
					return;
				} else if (txtXEnd.getText().trim().length() == 0) {
					JOptionPane.showMessageDialog(getContentPane(), "Enter X End!", "Error!", JOptionPane.WARNING_MESSAGE);
				} else if (txtYEnd.getText().trim().length() == 0) {
					JOptionPane.showMessageDialog(getContentPane(), "Enter Y End!", "Error!", JOptionPane.WARNING_MESSAGE);
				} else {
					try {
						...
						if (xS <= 0) {
							...
							line.getStart().setX(xS);
							line.getStart().setY(yS);
							line.getEnd().setX(xE);
							line.getEnd().setY(yE);
